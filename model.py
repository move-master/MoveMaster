...
def evaluate(policy, episodes=200):
    def run(mode="greedy"):
        ...
        for _ in range(episodes):
            ...
            ep_R = 0.0
            moves = 0
            ep_maxH = getattr(e, "height", 0)
            com_start = e._com_x_norm() if hasattr(e, "_com_x_norm") else 0.0
            ep_depth_sum = 0.0
            ep_two_count = 0.0
            ...
            while not done:
                ...
                ep_depth_sum += float(info2.get("pull_depth", 0))
                if info2.get("was_two_block", False):
                    ep_two_count += 1.0
            ...
            total_moves += moves
            total_maxH += ep_maxH
            total_dcom += abs(com_end - com_start)
            total_depth += ep_depth_sum
